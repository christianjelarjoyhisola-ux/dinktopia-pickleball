...
test("uses atomic multi-court checkout with responsive desktop and mobile matrices", async () => {
  ...
  assert.match(
    summarySource,
    /className="player-kicker">Your court hold[\s\S]*?court\{courts\.length === 1 \? "" : "s"\} held[\s\S]*?Complete your details below to continue to payment\.[\s\S]*?\{dateLabel\}[\s\S]*?\{slotLabel\}/,
  );
...
test("keeps the three-step checkout and confirmation compact, ordered, and complete on phones", async () => {
  ...
  assert.match(
    booking,
    /<dialog[\s\S]*?ref=\{holdIntroDialogRef\}[\s\S]*?aria-modal="true"[\s\S]*?Complete your details before the timer ends\.[\s\S]*?<HoldIntroSelectionSummary[\s\S]*?selections=\{selectedSlotDetails\}[\s\S]*?dateLabel=\{selectedBookingDateLabel\}[\s\S]*?subtotal=\{courtSubtotal\}[\s\S]*?bookingFee=\{bookingFee \?\? 0\}[\s\S]*?bookingFeeMode=\{bootstrap\?\.bookingFee\?\.feeMode\}[\s\S]*?bookingFeeAmount=\{bootstrap\?\.bookingFee\?\.feeAmount\}[\s\S]*?total=\{total\}[\s\S]*?formatHoldCountdown\(holdRemainingSeconds\)[\s\S]*?Continue to details[\s\S]*?<\/dialog>/,
  );
  assert.match(
    booking,
