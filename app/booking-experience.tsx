    ...
    <section className="club-gallery section-pad" id="gallery" aria-labelledby="gallery-heading">
      <div className="site-container">
        <div className="gallery-heading">
          ...
          <p>Fresh photos published by the {activeTenant.identity.shortName} team.</p>
        </div>
        ...
        {galleryPhotos.length ? (
          ...
          <div className="gallery-grid gallery-grid-placeholder" aria-label="Court gallery setup status">
            {["Court photos", "Venue moments", "More coming soon"].map((label, index) => (
    ...
    <div className={`dinktopia-site${isBookingPage ? " booking-route" : ""}${isBookingPage && mode === "book" ? " booking-new-route rallyos-player-shell player-mode" : ""}`}>
        ...
        {isHome && <section className="club-note">
          <div className="site-container club-note-inner community-hub">
            <div className="community-intro">
              ...
              <p>Club updates and community links will appear here after the {activeTenant.identity.shortName} team publishes them.</p>
            </div>
            <div className="community-links" aria-label="Community channels setup status">
              {["Club updates", "Instagram", "Facebook", "TikTok"].map((channel, index) => (
                <article className={`community-card${index === 0 ? " community-card-featured" : ""}`} key={channel}>
                  <span className="community-mark" aria-hidden="true">{index === 0 ? activeTenant.identity.shortName : channel.slice(0, 2).toUpperCase()}</span>
                  <span className="community-card-copy"><small>Coming soon</small><strong>{channel}</strong></span>
                  <span aria-hidden="true">•••</span>
                </article>
              ))}
            </div>
            ...
            {mode === "book" && !bookingSetupReady ? (
              <div className="setup-unavailable-card" role={checkingLiveSetup || !isLive ? "status" : "alert"}>
                ...
                <div>
                  ...
                  <h3>{checkingLiveSetup ? "Loading the court board…" : !isLive ? "Booking details are coming soon." : "The clubhouse is still getting ready."}</h3>
      ...
      <footer className="site-footer">
        <div className="site-container footer-grid">
          <div><TenantWordmark footer /><p>Good games live here.</p></div>
        ...
        <div className="site-container footer-bottom"><span>© 2026 {activeTenant.identity.name}</span><span>Made for longer rallies.</span></div>
      </footer>
